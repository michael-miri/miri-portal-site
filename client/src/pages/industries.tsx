          ...
          {sectors.map((sector, i) => (
            ...
            >
              <div className="lg:col-span-5">
                ...
                <p className="mt-4 text-[14px] text-white/30 leading-relaxed max-w-sm">{sector.desc}</p>
      ...
      <section className="py-32 bg-[#09090b] border-t border-white/[0.06]">
        <div className="max-w-[1200px] mx-auto px-6 sm:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            <div>
              <span className="text-[11px] font-medium tracking-[0.2em] uppercase text-white/30 block mb-6">More Industries</span>
              <h2 className="font-heading text-[clamp(2rem,3.5vw,3rem)] font-bold text-white leading-[1.08] tracking-tight">
                We also serve.
              </h2>
              <div className="mt-10">
                <Link href="/contact">
                  <Button className="bg-white text-black font-medium rounded-full px-7 h-11 hover:bg-white/90" data-testid="button-industries-contact">
            ...
            <div className="flex flex-wrap gap-2">
              {industryTags.map((tag) => (
                <span key={tag} className="text-[13px] px-4 py-2 rounded-full bg-white/[0.03] border border-white/[0.06] text-white/30">{tag}</span>
              ))}
            </div>
          </div>
        </div>
